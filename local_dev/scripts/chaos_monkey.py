...
def main(
    ...
    while True:
        ...
        if stopped_services and random.random() < starting_chance:
            ...
            subprocess.call(DOCKER_BASE_COMMAND + ["start", service], cwd=ROOT_DIR)
            print_ps = True
            ...
            subprocess.call(DOCKER_BASE_COMMAND + ["stop", service], cwd=ROOT_DIR)
            print_ps = True
        if print_ps:
            print("docker ps status:")
            subprocess.call(DOCKER_BASE_COMMAND + ["ps"], cwd=ROOT_DIR)
            print_ps = False
        else:
            print(
                "Not doing anything this iteration, just sleeping", sleep_time, "s..."
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--stopping-chance",
        type=float,
        help="Chance to stop a service (0 - 1.0)",
        default=0.1,
    )
    parser.add_argument(
        "--starting-chance",
        type=float,
        help="Chance to start a stopped service (0 - 1.0)",
        default=0.3,
    )
    parser.add_argument(
        "--sleep-time", type=float, help="Time to sleep between iterations", default=5
    )
